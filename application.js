...
const displayAllAdmissions = () => {
  ...
  // if (user['position']==='Doctor' || user['position']==="Nurse") {
    ...
    const log = admissions.length;
      while (i < log) {
      display.push(admissions[i].patientName + '    ' + admissions[i].attendingPhysician + '    ' + admissions[i].admitTime + '    ' + admissions[i].dischargeTime);
        i += 1;
    // }
...
$(document).ready(() => {
    ...
    const name = $('.login-form').find('input[name]').val();
    console.log(name)
    alert('hello')
    debugger
  })

  $('.displayAll').on('click', (event) => {
    $('.displaying-admissions').append(displayAllAdmissions())
  })
});
